...
export function IssueNode({ data }: NodeProps<IssueFlowNode>) {
  const { issue, category, rootRepo, dim, focus, selected } = data;
  const ready = category === "ready";
  return (
    <div
      className={cn(
        "relative flex h-full w-full items-center gap-1.5 overflow-hidden rounded-lg border-2 bg-card px-2.5 text-xs leading-snug text-foreground transition-opacity",
        category === "done" && "opacity-55",
        dim && "opacity-20",
        (focus || selected) && "ring-2 ring-ring ring-offset-2 ring-offset-background",
      )}
      style={{
        borderColor:
          category === "root"
    ...
    >
      ...
      <span className="line-clamp-2 min-w-0 flex-1 break-words">
        <span className="font-semibold text-muted-foreground">{refLabel(issue, rootRepo)}</span>{" "}
        {issue.title}
      </span>
      ...
      <a
        ...
        className="nodrag nopan shrink-0 text-muted-foreground hover:text-foreground"
