...
import { serve } from "../server/main.ts";
...
const USAGE = `usage: gh-conductor <command> <issue> [--repo owner/name] [--json]
       ...
       gh-conductor serve [--port N] [--from <graph.json>] [--dev] [--stop]
...
commands:
  ...
  view    <issue>  open the issue's graph in the browser (starts the local server if needed); prints the URL.
                   --no-open just prints.
  serve            run the graph server in the foreground. --from serves a saved graph for any issue.
                   --dev turns on Bun's HMR + browser console forwarding for UI work.
...
export async function main(argv: string[]): Promise<number> {
  const { values, positionals } = parseArgs({
    ...
    options: {
      ...
      idle: { type: "boolean", default: false }, // hidden: the detached daemon's own serve
  ...
  if (cmd === "serve") {
    ...
    const server = await serve({
      ...
      from: values.from,
      idle: values.idle,
      development: values.dev ? { hmr: true, console: true } : false,
    });
    // daemon.ts scrapes this line out of the detached child's log — keep the wording.
    ...
    await new Promise(() => {}); // until Ctrl-C, or the idle timeout
    return 0;
  }
