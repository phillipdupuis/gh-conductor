...
> **Stub.** Steps are placeholders; the executable protocol is still being derived from the dry-run scenarios.
...
**Owns (capability checklist):** track (agent-side) · prioritize (pick next) · unblock (the sweep) · deliver · evaluate (Done-when checks, proposing closes).

## Steps (skeleton)

1. **Read state from GitHub** — the graph, the ready frontier, what's waiting on a human.
