...
describe("layoutGraph (upgrade-python-mid, everything collapsed)", () => {
  ...
  const boxes: Box[] = [
    ...layout.issues,
    ...layout.layers.flatMap((l) => (l.node ? [l.node] : [])),
  ];

  test("one node per layer: plain issues for singletons, one list node otherwise", () => {
    expect(boxes.length).toBe(layers.length);
    expect(layout.layers.filter((l) => l.mode === "collapsed").map((l) => l.issues.length)).toEqual(
      [2, 2, 10],
    );
  ...
  test("a collapsed node caps at MAX_VISIBLE_ROWS and is centred like everything else", () => {
    ...
    for (const p of layout.issues)
      expect(p).toMatchObject({ x: -NODE_WIDTH / 2, width: NODE_WIDTH, height: NODE_HEIGHT });
  });
...
describe("layoutGraph (expanded)", () => {
  ...
  test("an expanded layer becomes framed columns with a centred toggle on the frame's top edge, nothing overlapping", () => {
    ...
    const boxes: Box[] = [
      ...layout.issues,
      ...layout.layers.flatMap((l) => [l.node, l.toggle].filter((b): b is Box => b !== null)),
    ];
    for (let i = 0; i < boxes.length; i++)
      for (let j = i + 1; j < boxes.length; j++) expect(overlaps(boxes[i]!, boxes[j]!)).toBe(false);
  });
  test("edges fan out to the individual issues", () => {
    expect(layout.edges.filter((e) => e.source === k(129) && e.kind === "blocking").length).toBe(
      10,
    );
  });
});
...
describe("boundsOf", () => {
  const row = (p: Partial<LayerPlacement>): LayerPlacement => ({
    layer: 0,
    issues: [],
    mode: "single",
    y: 0,
  ...
  test("covers every placed box: issue, list node, frame and toggle", () => {
    ...
    const layout: Layout = {
      ...
      layers: [
        ...
        row({
          layer: 1,
          frame: { x: -260, y: 0, width: 520, height: 80 },
          toggle: { x: -48, y: -22, width: 96, height: 28 },
        }), // minX = -260, minY = -22
      ],
      edges: [],
    };
  ...
  test("a layout that places nothing has zero bounds", () => {
    expect(boundsOf({ issues: [], layers: [], edges: [] })).toEqual({
      minX: 0,
      minY: 0,
      maxX: 0,
      maxY: 0,
    ...
    expect(boundsOf({ issues: [], layers: [row({}), row({ layer: 1 })], edges: [] })).toEqual({
      minX: 0,
      minY: 0,
      maxX: 0,
      maxY: 0,
  ...
  test("a single box's bounds are the box itself", () => {
    const layout: Layout = {
      issues: [{ key: k(7), layer: 0, x: -110, y: 64, width: 220, height: 56 }],
      layers: [],
      edges: [],
    };
