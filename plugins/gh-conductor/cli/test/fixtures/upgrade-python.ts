...
import type { Blocker, Graph, IssueState, Node, Pr } from "../../src/graph.ts";
...
export function upgradePython(stage: Stage): Graph {
  ...
  const walk = (parent: number): Node[] => (byParent.get(parent) ?? []).flatMap((s) => [node(s), ...walk(s.number)]);
  return {
    repo: REPO,
    viewer: VIEWER,
